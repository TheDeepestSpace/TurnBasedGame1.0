...
/**
 * Created by Boris on 23.03.2016.
 * Project: TurnBasedGame1.0
 */
public class IgnoreDirectionsToBoundsTask extends LeafTask<AI> implements InformableTaskInterface {
    ...
    void excludeDirection() {
        if (Entity.getEntity(entityFullName).getGridCoordinates().x + 1 >= Grid.size.x) {
            Actors.gameAI.excludedDirections[0] = true;
            excludedDirection = 0;
            informSucceeded();
        }
        if (Entity.getEntity(entityFullName).getGridCoordinates().x - 1 < 0) {
            Actors.gameAI.excludedDirections[2] = true;
            excludedDirection = 2;
            informSucceeded();
        }
        if (Entity.getEntity(entityFullName).getGridCoordinates().z + 1 >= Grid.size.z) {
            Actors.gameAI.excludedDirections[1] = true;
            excludedDirection = 1;
            informSucceeded();
        }
        if (Entity.getEntity(entityFullName).getGridCoordinates().z - 1 < 0) {
