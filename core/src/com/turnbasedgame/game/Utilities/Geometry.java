...
public class Geometry {
    public static Vector3 getSceneCoordinates(Vector3 gridCoordinates, String gridName) {
        Vector3 out = new Vector3();

        if (Grid.getGrid(gridName).size.x % 2 == 0) {
            out.x = gridCoordinates.x * Grid.tileSceneSize - Grid.getGrid(gridName).size.x / 2 + (float) Grid.tileSceneSize / 2;
        }else {
            out.x = gridCoordinates.x * Grid.tileSceneSize - Grid.getGrid(gridName).size.x / 2;
        }

        out.y = gridCoordinates.y * Grid.tileSceneSize;

        if (Grid.getGrid(gridName).size.z % 2 == 0) {
            out.z = gridCoordinates.z * Grid.tileSceneSize - Grid.getGrid(gridName).size.z / 2 + (float) Grid.tileSceneSize / 2;
        }else {
            out.z = gridCoordinates.z * Grid.tileSceneSize - Grid.getGrid(gridName).size.z / 2;
        }
